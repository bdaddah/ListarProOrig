...
const ProductDetail3: React.FC<ScreenContainerProps> = ({
  navigation,
  ...params
}) => {
  ...
  const buildContent = () => {
    ...
    if (product) {
      ...
      if (product.address) {
        const locations = [];
        if (product.country) {
          locations.push(product.country.title);
        }
        if (product.city) {
          locations.push(product.city.title);
        }
        if (product.state) {
          locations.push(product.state.title);
        }
        address = (
          <>
            <SizedBox height={Spacing.M} />
            ...
            <MapView
              ...
              <Marker
                coordinate={{
                  latitude: product.gps?.latitude!,
                  longitude: product.gps?.longitude!,
                }}
              />
            </MapView>
          </>
      ...
      if (product.fax) {
        fax = (
          <>
            ...
            <TouchableOpacity
              style={Styles.row}
              onPress={() => onAction(`tel:${product?.fax}`)}>
              <View
                style={[
                  styles.iconInfo,
                  {backgroundColor: theme.colors.background.disable},
                ]}>
                <Icon name={'fax'} size={18} />
              </View>
              <SizedBox width={Spacing.S} />
              <View style={Styles.flex}>
                <Text
          ...
          <>
            ...
            {showOpenHours && (
              <FlatList
                ...
                data={product?.openHours}
                renderItem={({item}) => {
                  return (
                    <View style={styles.subInfo}>
                      <Text typography="caption2">{translate(item.key)}</Text>
                      <Text
                        typography="caption2"
                        fontWeight="bold"
                        color={theme.colors.secondary.default}>
                ...
                keyExtractor={(item, index) => `time${item?.key ?? index}`}
